...
def do_eval(sess, model, log, rule_train):
    ...
    for rule_test in hp['rules']:
        ...
        for i_rep in range(n_rep):
            ...
            c_lsq, c_reg, y_hat_test,h,x = sess.run(
                [model.cost_lsq, model.cost_reg, model.y_hat,model.h,model.x],
                feed_dict=feed_dict)
...
def train(model_dir,
          ...
          load_dir=None,
          trainables=None,
          continue_after_target_reached=True, #if set to True, the training will continue for 50 more steps after reaching the target performance
          ):
    """Train the network.
